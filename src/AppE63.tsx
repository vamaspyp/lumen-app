...
function FieldChrome({ authenticated, go, onHome, onSignOut }: { authenticated: boolean; go: (space: Space) => void; onHome: () => void; onSignOut: () => void }) {
  return <header className="field-chrome"><button className="field-brand" type="button" onClick={onHome}>LUMEN</button><nav className="entry-links" style={{marginTop:0,justifyContent:'center'}} aria-label="Espacios de LUMEN"><button type="button" aria-label="Ahora" onClick={() => go('now')}>Ahora</button><button type="button" aria-label="Fuente" onClick={() => go('source')}>Explorar</button><button type="button" aria-label="Trayectoria" onClick={() => go('territory')}>Mi Vida</button><button type="button" onClick={() => go('sanctuary')}>Santuario</button><button type="button" onClick={() => go('tissue')}>Tejido</button></nav><button className="field-auth" type="button" onClick={authenticated ? onSignOut : onHome}>{authenticated ? 'Salir' : 'presencia'}</button></header>
}

function MovementLinks({ go }: { go: (space: Space) => void }) {
  return <div className="entry-links" aria-label="Moverme por LUMEN"><button type="button" onClick={() => go('source')}>explorar Fuente</button><button type="button" onClick={() => go('territory')}>mi cielo</button><button type="button" onClick={() => go('sanctuary')}>Santuario</button><button type="button" onClick={() => go('tissue')}>Tejido</button></div>
}

function PrivateGate({ name, go }: { name: string; go: (space: Space) => void }) {
  return <section className="field-scene narrow"><div className="presence-orb small" aria-hidden="true"/><p className="field-eyebrow">{name} · espacio personal</p><h1 aria-label={`${name} necesita saber que sos vos.`}>Necesito saber que sos vos.</h1><p className="field-copy">Este espacio puede contener memoria, vínculos o continuidad. LUMEN no mezcla vidas.</p>{name==='Tejido'&&<p className="field-copy">Los Círculos son privados y por invitación.</p>}<button className="field-primary" type="button" onClick={() => go('now')}>Volver a Ahora para entrar</button></section>
}
...
function SourceField({ authenticated, go }: { authenticated: boolean; go: (space: Space) => void }) {
  ...
  return <>
    <section className="field-scene">
      <div className="source-intro"><div><p className="field-eyebrow">FUENTE · reservorio vivo</p><h1 aria-label="Algo del patrimonio humano, cuando haga falta.">Sabiduría en muchas formas.</h1><p className="field-copy">No es un catálogo para consumir. Es un reservorio que puede abrir perspectivas, prácticas, obras, personas, lugares, acciones, ayuda concreta o quietud.</p></div><div className="source-controls"><label>Área<select value={area} onChange={(event) => { setBusy(true); setArea(event.target.value) }}><option value="">todas</option>{taxonomy?.areas.map((term) => <option key={term.key} value={term.key}>{term.label}</option>)}</select></label><label>Capacidad<select value={capacity} onChange={(event) => { setBusy(true); setCapacity(event.target.value); setType('') }}><option value="">todas</option>{taxonomy?.capacities.map((term) => <option key={term.key} value={term.key}>{term.label}</option>)}</select></label><label>Forma<select value={type} onChange={(event) => { setBusy(true); setType(event.target.value) }}><option value="">todas</option>{taxonomy?.help_types?.map((term) => <option key={term.key} value={term.key}>{term.label}</option>)}</select></label></div></div>
      ...
      {health && <p className="source-state">{health.source.active_possibilities} posibilidades activas limitadas · {health.source.semantic_types} formas semánticas · cobertura honesta y todavía limitada.</p>}
      ...
      <div className="possibility-grid">{items.map((item) => <div key={item.help_id} style={{gridColumn:'span 6'}}><PossibilityPreview help={item} onOpen={() => setSelected(item)} /><div style={{display:'flex',justifyContent:'flex-end'}}><button className="field-link" type="button" aria-label={saved.has(item.help_id)?'Guardado en Santuario':!memoryAllowed&&authenticated?'Permitir memoria y guardar':authenticated?'Guardar en Santuario':'Entrar para guardar'} disabled={busy || saved.has(item.help_id)} onClick={() => void save(item)}>{saved.has(item.help_id) ? 'guardado' : authenticated ? 'guardar para volver' : 'entrar para guardar'}</button></div></div>)}</div>
...
function TerritoryField({ go }: { go: (space: Space) => void }) {
  ...
  useEffect(() => { refresh().catch((cause) => setError(cause instanceof Error ? cause.message : 'No pude abrir tu cielo.')).finally(() => setBusy(false)) }, [])
...
function SanctuaryField({ go }: { go: (space: Space) => void }) {
  ...
  const [snapshot, setSnapshot] = useState<ContinuitySnapshot | null>(null)
  const [title,setTitle] = useState(''); const [content,setContent] = useState(''); const [busy,setBusy] = useState(true); const [error,setError] = useState(''); const [message,setMessage] = useState('')
  const refresh = async () => { const [c,e] = await Promise.all([getContinuitySnapshot(),listSanctuary()]); setSnapshot(c); setEntries(e) }
  useEffect(() => { refresh().catch((cause)=>setError(cause instanceof Error?cause.message:'No pude abrir Santuario.')).finally(()=>setBusy(false)) },[])
...
function TissueField({ go }: { go:(space:Space)=>void }) {
  ...
  useEffect(()=>{refresh().catch((cause)=>setError(cause instanceof Error?cause.message:'No pude abrir Tejido.')).finally(()=>setBusy(false))},[])
  const act=async(operation:()=>Promise<unknown>)=>{setBusy(true);setError('');try{await operation();await refresh()}catch(cause){setError(cause instanceof Error?cause.message:'No pude completar ese gesto.')}finally{setBusy(false)}}
...
export default function AppE63() {
  ...
  return <main className="lumen-field"><FieldChrome authenticated={authChecked&&authenticated} go={go} onHome={()=>go('now')} onSignOut={()=>void logout()}/>
    {space==='source'&&<SourceField authenticated={authenticated} go={go}/>} {space==='territory'&&(authenticated?<TerritoryField go={go}/>:<PrivateGate name="Mi Vida" go={go}/>)} {space==='sanctuary'&&(authenticated?<SanctuaryField go={go}/>:<PrivateGate name="Santuario" go={go}/>)} {space==='tissue'&&(authenticated?<TissueField go={go}/>:<PrivateGate name="Tejido" go={go}/>)}
    {space==='now'&&<section className="field-scene narrow" aria-live="polite"><div className="presence-orb" aria-hidden="true"/>
      {(stage==='home'||stage==='auth')&&<><p className="field-eyebrow">LUMI · presencia</p><h1 aria-label="Estoy acá.">¿Cómo estás hoy?</h1><p className="field-copy">Podés contarme lo que está presente en tu vida, como te salga. No hace falta elegir una categoría.</p><form className="moment-box" onSubmit={(e)=>{e.preventDefault();void submitMoment(expression)}}><textarea aria-label="Lo que te está pasando" value={expression} onChange={(e)=>setExpression(e.target.value)} maxLength={4000} disabled={busy||stage==='auth'} placeholder="Contame qué está presente en tu vida…"/><div className="moment-actions">{stage==='home'&&<button className="field-primary" disabled={busy||!expression.trim()}>Ver qué podría ayudarme</button>}</div></form>{stage==='home'&&<><MovementLinks go={go}/><p className="privacy-soft">Tu expresión original se conserva en privado para que LUMEN pueda reinterpretarla mejor cuando aprenda. No se copia al Ledger ni se usa como aprendizaje compartido por defecto.</p></>}{stage==='auth'&&<div className="field-panel auth-panel"><h2>Antes de seguir</h2><p>Para sostener este encuentro sin mezclar vidas, necesito que entres con un correo. Si preferís no hacerlo, Fuente sigue abierta sin identificarte.</p>{magicLinkSent?<p className="success-field">Te envié un enlace de acceso.</p>:<form onSubmit={sendMagic}><input aria-label="Tu correo" className="field-input" type="email" value={email} onChange={(e)=>setEmail(e.target.value)} required placeholder="tu correo"/><button className="field-primary" disabled={busy}>Enviarme un enlace</button><button className="field-ghost" aria-label="Ahora no · explorar sin entrar" type="button" onClick={()=>{resetHome();setSpace('source')}}>explorar Fuente sin entrar</button></form>}</div>}</>}
      {stage==='scene'&&scene&&<><p className="field-eyebrow">LUMI · {scene.presence_mode}</p><h1>{scene.scene_id==='moment.help'?(primaryHelp?.from_own_repertoire?'Tal vez no haga falta algo nuevo.':'Quizá podamos empezar por acá.'):scene.scene_id==='moment.clarify'?'Quiero entender un poco mejor.':'Prefiero ser claro con esto.'}</h1><p className="field-copy">{semanticCopy(scene)}</p>{scene.scene_id==='moment.help'&&primaryHelp&&<div style={{width:'min(560px,100%)'}}><PossibilityPreview help={primaryHelp} onOpen={()=>void choose('selected')}/><div className="panel-actions"><button className="field-primary" aria-label={primaryHelp.from_own_repertoire?'Volver a esto':'Quiero probarlo'} disabled={busy} onClick={()=>void choose('selected')}>{primaryHelp.from_own_repertoire?'volver a esto':'quiero vivirlo'}</button><button className="field-ghost" disabled={busy} onClick={()=>void choose('rejected')}>no es esto</button></div></div>}{scene.scene_id==='moment.clarify'&&<form className="moment-box" onSubmit={(e)=>{e.preventDefault();const extra=clarification.trim();if(extra)void submitMoment(`${expression.trim()}\n${extra}`)}}><textarea value={clarification} onChange={(e)=>setClarification(e.target.value)} placeholder="Sólo algo que cambie la ayuda…"/><div className="moment-actions"><button className="field-primary" disabled={busy||!clarification.trim()}>contarte un poco más</button><button className="field-ghost" type="button" onClick={()=>resetHome()}>cerrar</button></div></form>}{scene.scene_id==='moment.no_match'&&<div className="panel-actions"><button className="field-primary" onClick={()=>resetHome()}>contarlo de otra manera</button><button className="field-ghost" onClick={()=>resetHome('Con esto alcanza por ahora.')}>cerrar</button></div>}{scene.scene_id==='moment.safety_referral'&&<div className="safety-panel"><p>Buscá ahora a una persona de confianza, un profesional o un servicio de emergencia de tu zona. Si hay peligro inmediato, priorizá la ayuda humana presencial.</p><button className="field-primary" onClick={()=>resetHome('Ojalá puedas acercarte a alguien ahora. LUMEN queda acá, sin reemplazar esa ayuda.')}>entendido</button></div>}</>}
      {stage==='outcome'&&selectedHelp&&<div className="outcome-field"><p className="field-eyebrow">VOLVER</p><h1>¿Te ayudó algo de esto?</h1><p className="field-copy">Una señal breve alcanza. Missing sigue siendo missing; no hace falta responder.</p><div className="outcome-actions"><button aria-label="Sí, un poco" disabled={busy} onClick={()=>void finish('helped')}>sí, un poco</button><button disabled={busy} onClick={()=>void finish('not_helped')}>no era para mí</button><button disabled={busy} onClick={()=>void finish('unsure')}>no sé todavía</button></div></div>}
      {stage==='closed'&&<div className="outcome-field"><p className="field-eyebrow">LUMI · integrar o soltar</p><h1>{closing}</h1><p className="field-copy">Podés volver a tu vida. Si algo merece continuidad, sólo vos decidís qué conservar.</p>{lastOutcome==='helped'&&selectedHelp&&!integrated&&<button className="field-ghost" aria-label={`Guardar “${selectedHelp.title}” en mi repertorio`} disabled={busy} onClick={()=>void integrate()}>hacerlo parte de mi Repertorio</button>}{integrated&&<p className="success-field">Quedó en tu repertorio. LUMEN puede devolvértelo antes de sumar novedad.</p>}<MovementLinks go={go}/><button className="field-primary" onClick={()=>resetHome()}>volver a Ahora</button></div>}
      {error&&<p className="error-field" role="alert">{error}</p>}</section>}
    {stage==='experience'&&selectedHelp&&<ExperienceSurface help={selectedHelp} onExit={()=>setStage('outcome')}/>}</main>
}
