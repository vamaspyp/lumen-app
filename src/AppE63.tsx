...
function MovementLinks({ go }: { go: (space: Space) => void }) {
  return <div className="entry-links" aria-label="Moverme por LUMEN"><button type="button" onClick={() => go('source')}>Explorar</button><button type="button" onClick={() => go('territory')}>Mi proceso</button><button type="button" onClick={() => go('tissue')}>Comunidad</button></div>
}
...
function PremiumNav({ space, go }: { space: Space; go: (space: Space) => void }) {
  const items: Array<{ id: Space; label: string; icon: 'home' | 'compass' | 'sprout' | 'people' | 'book' }> = [
    ...
    { id: 'tissue', label: 'Comunidad', icon: 'people' },
...
function SourceField({ authenticated, go }: { authenticated: boolean; go: (space: Space) => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([sourcePromise, getSourceTaxonomy(), getEmbryoHealth(), authenticated ? getContinuitySnapshot() : Promise.resolve(null)])
      ...
      .catch((cause) => { if (!cancelled) setError(cause instanceof Error ? cause.message : 'No pude abrir Explorar.') })
  ...
  return <>
    <section className="field-scene">
      <div className="source-intro"><div><p className="field-eyebrow">EXPLORAR · contenido curado</p><h1>Explorar</h1><p className="field-copy">Descubrí contenido para una vida más tuya. Perspectivas, prácticas y recursos curados para elegir con calma.</p></div><div className="source-controls"><label>Área<select value={area} onChange={(event) => setArea(event.target.value)}><option value="">todas</option>{taxonomy?.areas.map((term) => <option key={term.key} value={term.key}>{term.label}</option>)}</select></label><label>Capacidad<select value={capacity} onChange={(event) => { setCapacity(event.target.value); setType('') }}><option value="">todas</option>{taxonomy?.capacities.map((term) => <option key={term.key} value={term.key}>{term.label}</option>)}</select></label><label>Forma<select value={type} onChange={(event) => setType(event.target.value)}><option value="">todas</option>{taxonomy?.help_types?.map((term) => <option key={term.key} value={term.key}>{term.label}</option>)}</select></label></div></div>
      ...
      {!busy && items.length === 0 && <p className="muted">No encontré algo suficientemente pertinente con esta combinación. Explorar conserva NO_MATCH.</p>}
      <MovementLinks go={go}/>
    </section>
...
function TerritoryField({ go }: { go: (space: Space) => void }) {
  ...
  useEffect(() => { setBusy(true); refresh().catch((cause) => setError(cause instanceof Error ? cause.message : 'No pude abrir Mi proceso.')).finally(() => setBusy(false)) }, [])
...
function TissueField({ go }: { go:(space:Space)=>void }) {
  ...
  useEffect(()=>{setBusy(true);refresh().catch((cause)=>setError(cause instanceof Error?cause.message:'No pude abrir Comunidad.')).finally(()=>setBusy(false))},[])
  const act=async(operation:()=>Promise<unknown>)=>{setBusy(true);setError('');try{await operation();await refresh()}catch(cause){setError(cause instanceof Error?cause.message:'No pude completar ese gesto.')}finally{setBusy(false)}}
...
export default function AppE63() {
  ...
  return <main className="lumen-field"><FieldChrome authenticated={authChecked&&authenticated} onHome={()=>go('now')} onSignOut={()=>void logout()}/>
    {space==='source'&&<SourceField authenticated={authenticated} go={go}/>} {space==='territory'&&(authenticated?<TerritoryField go={go}/>:<PrivateGate name="Mi proceso" go={go}/>)} {space==='sanctuary'&&(authenticated?<SanctuaryField go={go}/>:<PrivateGate name="Santuario" go={go}/>)} {space==='tissue'&&(authenticated?<TissueField go={go}/>:<PrivateGate name="Comunidad" go={go}/>)}
