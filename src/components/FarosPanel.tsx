...
function FaroCard({
  ...
  const subtitle = faro.area_label || faro.area || faro.area_key || ''
  ...
  return (
    ...
    >
      {subtitle && (
        <div style={{ fontSize: '0.68rem', color: tokens.textMuted, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.3rem' }}>
          {subtitle}
        </div>
      )}
...
export function FarosPanel({
  ...
  // Agrupación puramente visual (activos/pausados/cerrados) — no decide
  // qué actions existen, solo ordena la presentación.
  const knownOrder = ['active', 'paused', 'closed']
  const groups = knownOrder
    .map(status => ({ status, label: STATUS_LABELS[status], items: faros.filter(f => f.status === status) }))
    .filter(g => g.items.length > 0)
  const otherItems = faros.filter(f => !knownOrder.includes(f.status || ''))
  if (otherItems.length) groups.push({ status: 'other', label: 'Otros', items: otherItems })
    ...
    <div style={{ textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {groups.map(group => (
        <div key={group.status}>
          <div style={{ fontSize: '0.7rem', color: tokens.accentDeep, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 500, marginBottom: '0.6rem' }}>
            {group.label}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
            {group.items.map((faro, idx) => (
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            {availableTags.map((tag, idx) => (
              <Pill
                key={`${tag.faro_key}-${idx}`}
                label={tag.label || String(tag.faro_key)}
                hint={tag.hint}
                variant="outline"
