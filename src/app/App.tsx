...
function StartWays({ go,setExpression }: { go:(space:Space)=>void; setExpression:(value:string)=>void }) {
  ...
  return <section className="start-ways">
    <div className="start-copy"><b>ALGUNAS FORMAS<br/>DE COMENZAR</b><p>Diferentes entradas,<br/>un mismo lugar.</p></div>
...
export default function App() {
  ...
  const content=useMemo(()=>{
    ...
    if(space==='home')return <><HomeHero expression={expression} setExpression={setExpression} submit={()=>void runMoment()} busy={busy} authenticated={authenticated}/><StartWays go={go} setExpression={setExpression}/></>
    if(space==='explore')return <ExploreView source={source} taxonomy={taxonomy} authenticated={authenticated} onOpen={(item)=>void openSource(item)} onSave={saveSource}/>
    ...
    if(space==='sanctuary')return <SanctuaryView entries={entries} refresh={loadPrivate}/>
    if(space==='tissue')return <TissueView circles={circles} source={source} refresh={loadPrivate} onOpen={(item)=>void openSource(item)}/>
