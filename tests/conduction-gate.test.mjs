...
test('conduction context is explicit and fail-closed', () => {
  ...
  for (const required of ['V46','V51','V52','V53','V41','V42']) assert.ok(authorityIds.has(required));
  for (const superseded of ['V48','V49']) assert.equal(authorityIds.has(superseded), false);
  assert.match(context.construction_rule ?? '', /preserva.*ajusta.*reemplaza/i);
  assert.match(context.construction_rule ?? '', /bridges permanentes/i);
  assert.deepEqual(context.required_sequence, ['POV_ACTIVOS','FOCO','ACTO_DOD','AUTORIDAD_VIGENTE','REALIDAD_TECNICA','EJECUCION','EVIDENCIA','CIERRE']);
});
...
test('official agent entrypoints inherit the conduction gate', () => {
  for (const path of ['AGENTS.md','CLAUDE.md','.github/copilot-instructions.md']) {
    ...
    assert.match(text, /CONDUCTION_GATE\.md/i);
    assert.match(text, /FAIL CLOSED/i);
    assert.match(text, /conduction-context\.json/i);
  }
});
...
test('governance enforcement surfaces have human custody', () => {
  ...
  for (const target of ['/governance/','/.github/workflows/','/scripts/check-conduction-gate.mjs','/tests/conduction-gate.test.mjs','/AGENTS.md','/CLAUDE.md']) assert.match(owners, new RegExp(target.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
...
test('standalone conduction gate check passes', () => {
  execFileSync(process.execPath, ['scripts/check-conduction-gate.mjs'], { stdio: 'pipe', env: { ...process.env, GITHUB_EVENT_NAME: '' } });
});
...
test('push commit identifies the same ACTO as conduction context', () => {
  ...
  assert.match(message.toUpperCase(), new RegExp(`\\b${context.act.id.toUpperCase()}\\b`));
});
