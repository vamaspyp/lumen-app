...
test('greenfield source never imports legacy application folders', async () => {
  ...
  const forbidden = [/(?:^|\/)components(?:\/|$)/,/(?:^|\/)lib(?:\/|$)/,/(?:^|\/)legacy(?:\/|$)/]
  for (const file of files.filter((p) => /\.(?:ts|tsx|js|mjs)$/.test(p))) {
    const source = await readFile(file, 'utf8')
    const imports = [...source.matchAll(/(?:from\s+|import\s*\()(['"])([^'"]+)\1/g)].map((m) => m[2])
    for (const specifier of imports) for (const pattern of forbidden) assert.equal(pattern.test(specifier), false, `${relative(ROOT, file)} imports forbidden legacy path ${specifier}`)
  }
})
...
test('greenfield migrations reference only explicit greenfield public API objects', async () => {
  ...
  for (const file of files) {
    ...
    for (const match of sql.matchAll(/\bpublic\.([a-zA-Z_][a-zA-Z0-9_]*)/g)) assert.ok(match[1].startsWith('lumen_'), `${relative(ROOT, file)} references non-greenfield public object public.${match[1]}`)
...
test('runtime entrypoints depend only on clean-room app, greenfield or framework code', async () => {
  const paths = [join(ROOT, 'src', 'main.tsx'), ...await walk(APP)]
  for (const path of paths.filter((p) => /\.(?:ts|tsx)$/.test(p))) {
    ...
    assert.equal(/master-(?:react|exact)/.test(source), false, `${relative(ROOT, path)} imports a superseded UI version`)
  }
})