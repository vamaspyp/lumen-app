...
test('Premium home preserves the canonical mobile composition', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await installRpcMocks(page, [])
  await page.goto('/')
  await expect(page.getByRole('heading', { name: '¿Cómo estás hoy?' })).toBeVisible()
  await expect(nav(page).getByRole('button', { name: 'Inicio', exact: true })).toBeVisible()
  await expect(nav(page).getByRole('button', { name: 'Explorar', exact: true })).toBeVisible()
  await expect(nav(page).getByRole('button', { name: 'Mi proceso', exact: true })).toBeVisible()
  await expect(nav(page).getByRole('button', { name: 'Comunidad', exact: true })).toBeVisible()
  await expect(nav(page).getByRole('button', { name: 'Santuario', exact: true })).toBeVisible()
  await page.screenshot({ path: 'test-results/cleanroom-home-mobile.png', fullPage: true })
})
...
test('authenticated person can move through Mi proceso, Explorar, Santuario and Comunidad in the same Field', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Un camino hacia una vida más tuya.' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Juntos también se avanza.' })).toBeVisible()
