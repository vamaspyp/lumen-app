...
test('approved Premium design is the only public runtime shell', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByRole('heading', { name: 'Recuperar mi espacio' })).toBeVisible()
  await expect(nav(page)).toBeVisible()
  await expect(nav(page).getByRole('button', { name: 'Mi proceso', exact: true })).toBeVisible()
  ...
  await expect(nav(page).getByRole('button', { name: 'Comunidad', exact: true })).toBeVisible()
  await expect(page.getByText('PARA EMPEZAR HOY', { exact: true })).toBeVisible()
  await page.screenshot({ path: 'test-results/cleanroom-home.png', fullPage: true })
})

test('authenticated person can move through Mi proceso, Explorar, Santuario and Comunidad in the same Field', async ({ page }) => {
  const calls: string[] = []
  await installSession(page)
  await installRpcMocks(page, calls)
  await page.goto('/')
  await nav(page).getByRole('button', { name: 'Mi proceso', exact: true }).click()
  ...
  await nav(page).getByRole('button', { name: 'Comunidad', exact: true }).click()
...
test('Capacidades compose a real constellation instead of filtering a painted catalogue', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Regulación', exact: true })).toBeVisible()
...
test('regulation_premium_v1 is visible and can be lived end to end inside the LUMEN field', async ({ page }) => {
  ...
  for (const item of premiumSourceItems) await expect(page.getByRole('heading',{name:item.title,exact:true}).first()).toBeVisible()
  await page.locator('.premium-source-card').filter({hasText:'Poner los pies en la tierra · audio OPS/OMS'}).last().getByRole('button',{name:'Vivir esta posibilidad'}).click()
